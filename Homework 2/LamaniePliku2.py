...
(plik dictionary2.txt) zostal umieszczony w folderze z rozwiazaniami, aby umozliwic realizacje reszty kodu bez potrzeby
dolaczania przez Pana odm.txt do tego folderu.
'''

'''
source = open('odm.txt', "r")
dic = open('dictionary2.txt', 'a')
excluded = (".", "-", "'", " ", "à", "é", "ç", "ô", "ü", "Ł", "ö")
...
dic = open('dictionary2.txt', 'r')
hashDic = open('hash_dictionary2.txt', 'a')
...
while True:
    ...
    for i in range(0, len(line)):
        if line[i] != "\n":
            ...
            break
    hashDic.write(hashlib.md5(word.decode('utf-8').encode('utf-8')).hexdigest() + "\n")
dic.close()
hashDic.close()
...
def find_word(hash_word):
    ...
    line = None
    found = False
    dic = open('hash_dictionary2.txt', 'r')
